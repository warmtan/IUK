...
void RobotModel::getAg_AgdotQdot(MatrixXd & Ag, MatrixXd & AgdotQdot){
  ...
  MatrixXd joint_lin_mat = H.block(0,6,3,7);
  MatrixXd joint_ang_mat = H.block(3,6,3,7);
  //moemntum: rpy xyz
  //vel: rpy xyz joint
  ...
  H.block(0,6,3,7) = joint_ang_mat;
  H.block(3,6,3,7) = joint_lin_mat;
  ...
  Ag.block(0,0,3,13) = (base_X_com.toMatrixTranspose() * body_X_base.toMatrixTranspose() * U1 * H).block(0,0,3,13);
